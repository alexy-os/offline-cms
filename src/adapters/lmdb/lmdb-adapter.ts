...
import { DEFAULT_LMDB_PATH } from './paths';
...
/**
 * LMDB storage adapter for caching site data.
 * Used as MAINDB for fast offline access.
 */
export class LmdbAdapter<TCollections extends CollectionsShape = CollectionsShape>
  implements StorageAdapter<TCollections>
{
  public name = 'lmdb';
  private db: any;

  constructor(path = process.env.LMDB_DATA_PATH || DEFAULT_LMDB_PATH) {
    this.db = open({
      path,
      compression: true,
      maxDbs: 10,
  ...
  async saveAll(data: TCollections): Promise<void> {
    ...
    // Replace content (overwrite by key)
